...
from model_data_process.dataset import BiMEmpDialoguesDataset
...
from settings import DEFAULT_SAVE_DIR_PREFIX, HUB_ACCESS_TOKEN, \
    HUB_BIMODEL_ID, HUB_BIMODEL_PRIVATE_REPO, BMEDIALOGUES_PATH
...
class TrainInterface(BaseInterface):
    ...
    def _run_main_process(self):
        ...
        train_dataset = BiMEmpDialoguesDataset(dataset_dir=BMEDIALOGUES_PATH, split='train', transform=self.TRANSFORMS)
        val_dataset = BiMEmpDialoguesDataset(dataset_dir=BMEDIALOGUES_PATH, split='validation',
                                             transform=self.TRANSFORMS)

        model_class = models.MultiModalResponseGenerator
        try:
            model = model_class.from_pretrained(HUB_BIMODEL_ID, token=HUB_ACCESS_TOKEN)
        ...
        trainer = Seq2SeqTrainer(
            ...
            compute_metrics=Metrics(tokenizer=self.CONVERSATION_TOKENIZER.tokenizer,
                                    task_list=['text_generator', ]).compute,
